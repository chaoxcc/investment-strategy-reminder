...
from history_manager import add_operation, load_us_redemption_status, save_us_redemption_status


def get_simulated_triggers(simulate_rule):
    simulated_messages = {
        'a_sell': [('A股赎回', '创业板指近5日涨幅 +8.5%，请赎回融通创业板 + 广发中证500 各1/3')],
        'us_sell': [('美股赎回', '纳斯达克100近5日跌幅 -6.2%，请赎回嘉实美国成长 + 广发全球精选 各1/3（趋势破位避险）')],
        ...
        'full': [
            ...
            ('A股赎回', '创业板指近5日涨幅 +8.7%，请赎回融通创业板 + 广发中证500 各1/3'),
            ('美股赎回', '纳斯达克100近5日跌幅 -5.3%，请赎回嘉实美国成长 + 广发全球精选 各1/3（趋势破位避险）')
        ],
    }
...
def main():
    ...
    for rule, msg in triggers:
        add_operation(rule, msg)
        if rule == '美股回补-第一阶段':
